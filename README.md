...
| 항목 | 현황 | 비고 |
|------|------|------|
| 네트워크 격리 | 없음 | sandbox 내 코드가 외부 API 호출 가능 — 의도된 동작 |
| 프로세스 수 제한 | 없음 | sandbox 코드가 다수 subprocess spawn 가능. `RLIMIT_NPROC`으로 제한 가능하나 non-root 환경 필요. K8s `pids-limit`이 더 확실 |
| 디스크 격리 | 부분 | 세션별 디렉토리 분리 + 용량 제한. Pod 디스크 자체는 공유 |
